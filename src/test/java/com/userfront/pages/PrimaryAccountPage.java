...
/**
 * 主账户页面对象类
 * 继承自HomePage基类，封装了主账户页面的所有元素定位和操作方法
 * 页面URL: /account/primaryAccount
 */
public class PrimaryAccountPage extends HomePage {

    /**
     * 主账户余额显示元素
     * 包含人民币符号和余额数值
     */
    @FindBy(xpath = "//div[contains(@class,'panel-info')]//h1")
    private WebElement accountBalance;

    /**
     * 账户余额文本（不含图标）
     */
    @FindBy(xpath = "//div[contains(@class,'panel-info')]//h1//span")
     ...
     */
    public PrimaryAccountPage(WebDriver driver) {
        super(driver);
    }
     ...
     * @return 当前位置是否在主账户页面
     */
    public boolean isPrimaryAccountPageDisplayed() {
        return driver.getCurrentUrl().contains("/account/primaryAccount");
    }
}